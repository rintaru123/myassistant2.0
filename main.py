...
class LocalizationManager(QObject):
    ...
    def _ensure_locales_exist(self):
        ...
        if not os.path.exists(ru_path):
            ru_data = {
                "app_title": "Ассистент 2.0", "lang_name": "Русский", "add_task_button": "Добавить",
                ...
                "delete_list_confirm": "Вы уверены, что хотите удалить список '{list_name}'?",
                ...
                "settings_browse_btn": "Обзор...", "settings_clear_btn": "Очистить", "settings_transparent_editor": "Прозрачный редактор",
                ...
                "left_column_toggle": "Список", "left_column_tooltip": "Показать/скрыть список заметок",
                "right_column_toggle": "Задачи", "right_column_tooltip": "Показать/скрыть список задач",
                ...
                "settings_enable_tree": "Древовидные заметки (beta)", "tree_new_folder": "Новая папка...",
                "tree_rename_folder": "Переименовать папку...", "tree_delete_folder": "Удалить папку",
                "tree_delete_note": "Удалить заметку", "tree_new_note_here": "Новая заметка здесь",
                "tree_confirm_delete_folder": "Удалить папку '{name}' со всем содержимым?",
                "settings_audio_folder_label": "Папка музыки:", "audio_toggle_tooltip": "Музыка", "audio_prev": "Предыдущий",
                "audio_next": "Следующий", "audio_play": "Воспроизвести", "audio_pause": "Пауза", "audio_stop": "Стоп",
                "audio_volume": "Громкость", "settings_zen_audio_folder_label": "Музыка Zen:",
        ...
        if not os.path.exists(en_path):
            en_data = {
                "app_title": "Assistant 2.0", "lang_name": "English", "add_task_button": "Add",
                ...
                "export_menu": "Export Notes to Markdown...", "restore_menu": "Restore from Backup...", "exit_menu": "Exit",
                ...
                "settings_browse_btn": "Browse...", "settings_clear_btn": "Clear", "settings_transparent_editor": "Transparent editor",
                ...
                "settings_first_line_indent": "1st line indent (px):", "task_menu_edit": "Edit...",
                "task_menu_toggle_completed": "Toggle completed", "note_pin_menu": "Pin", "note_unpin_menu": "Unpin",
                "list_management_tooltip": "Right-click to manage lists", "open_window_menu": "Open window mode…",
                "open_window_tooltip": "Open in window mode", "left_column_toggle": "List",
                "left_column_tooltip": "Show/hide notes list", "right_column_toggle": "Tasks",
                "right_column_tooltip": "Show/hide tasks", "to_panel_button": "⇦ Panel",
                ...
                "task_templates_hint": "One template per line:", "settings_enable_tree": "Tree notes (beta)",
                ...
                "settings_audio_folder_label": "Music folder:", "audio_toggle_tooltip": "Music", "audio_prev": "Previous",
                "audio_next": "Next", "audio_play": "Play", "audio_pause": "Pause", "audio_stop": "Stop",
                "audio_volume": "Volume", "settings_zen_audio_folder_label": "Music Zen:", "new_note_title": "New Note",
...
class SettingsPanel(QWidget):
    ...
    def connect_signals(self):
        ...
        self.backup_interval_spin.valueChanged.connect(self.apply_changes)
        self.backup_max_count_spin.valueChanged.connect(self.apply_changes)
        
        if hasattr(self.parent().data_manager, 'create_backup'):
            self.create_backup_btn.clicked.connect(lambda: self.parent().data_manager.create_backup(notify=True))
...
class ZenModeWindow(QWidget, ThemedMenuMixin):
    ...
    def __init__(self, initial_text, settings, loc_manager, data_manager):
        ...
        self.settings_panel = SettingsPanel(self.settings, self.loc, self, context="zen_mode")
    ...
    def _update_styles(self):
        ...
        editor_bg = QColor(self.settings.get(bg_key))
        ...
        stylesheet = f"""
            QTextEdit {{
                background-color: {editor_bg_rgba}; border: none; font-family: '{font_family}';
                font-size: {font_size}pt; color: {editor_color};
                padding-top: {self.settings.get("editor_padding_top", 8)}px;
                padding-bottom: {self.settings.get("editor_padding_bottom", 8)}px;
                padding-left: {self.settings.get("editor_padding_left", 10)}px;
                padding-right: {self.settings.get("editor_padding_right", 10)}px;
            }}
            QWidget#bottomPanel {{ background-color: transparent; border-top: 1px solid {border_color}; }}
        ...
        if self.global_audio_widget:
            self.global_audio_widget.apply_zen_style(
                floating_fg=floating_fg, component_bg=component_bg,
...
class MainPopup(QWidget, ThemedMenuMixin):
    ...
    def __init__(self, data_manager):
        ...
        self.settings_panel_main = SettingsPanel(
            self.data_manager.get_settings(), self.loc, self, context="main_popup"
        )
...
class WindowMain(QWidget, ThemedMenuMixin):
    ...
    def __init__(self, data_manager):
        ...
        self.settings_panel_main = SettingsPanel(
            self.data_manager.get_settings(), self.loc, self, context="window_main"
        )
        self.settings_panel_main.settings_changed.connect(self.data_manager.update_settings)
        self.settings_panel_main.hide()
        self.settings_panel_main.installEventFilter(self)
    ...
    def _on_splitter_moved(self, pos, index):
        ...
        self.splitter_sizes_changed.emit(sizes)

    def _restore_splitter_sizes(self):
        sizes = self.data_manager.get_settings().get("window_splitter_sizes", [250, 500, 350])
        if len(sizes) == 3: self.splitter.setSizes(sizes)
    ...
    def _restore_window_state_or_set_ratio(self, force_recalc=False):
        ...
        if sides := st.get("window_splitter_sizes"):
            try:
                if len(sides) == 3:
                    self.splitter.setSizes(sides)
                    return
            except Exception: pass
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            self.splitter_save_timer.timeout.emit()
    ...
    def apply_zen_style(self, floating_fg, component_bg, hover_bg, border_color, accent):
        stylesheet = f"""
            ...
            QPushButton#playlistLabel {{
                color: {floating_fg}; background-color: transparent; border: none;
...
class TriggerButton(QWidget):
    ...
    def enter_zen_mode(self, initial_text, timestamp):
        ...
        self.zen_return_to_window_mode = bool(self.main_window and self.main_window.isVisible())
    ...
    def update_settings(self, new_settings):
        ...
        if self.main_popup: self.main_popup.apply_theme(new_settings); self.main_popup.retranslate_ui()
        if self.main_window: self.main_window.apply_theme(new_settings); self.main_window.retranslate_ui()
        if self.zen_window: self.zen_window.update_zen_settings(new_settings)
    ...
    def create_backup(self, notify=False):
        ...
        if os.path.exists(DATA_FILE):
            ...
            try:
                ...
                all_backups = sorted(glob(os.path.join(BACKUP_DIR, "data_*.bak")))
    ...
    def _load_and_validate_data(self):
        ...
        if "notes" not in data or not isinstance(data["notes"], list): data["notes"] = []; data_changed = True
        if "note_tree" not in data or not isinstance(data["note_tree"], list): data["note_tree"] = []; data_changed = True
        if "task_lists" not in data: data["task_lists"] = {"Default": []}; data_changed = True
        ...
        if data_changed:
            ...
            except Exception as e: print(f"Error saving validated data: {e}")
    ...
    def _update_ui_from_cache(self, container):
        ...
        if container.isVisible(): container.set_status_saved()

    def _collect_tree_timestamps(self, tree_list):
        out = set()
        for node in tree_list or []:
            if node.get("type") == "note":
                if ts := node.get("timestamp"): out.add(ts)
            elif node.get("type") == "folder": out |= self._collect_tree_timestamps(node.get("children", []))
        return out
    ...
    def _filter_tree_by_valid_ts(self, tree_list, valid_ts_set):
        ...
        for node in tree_list or []:
            if node.get("type") == "note":
                if node.get("timestamp") in valid_ts_set: result.append(node)
            elif node.get("type") == "folder":
                new_children = self._filter_tree_by_valid_ts(node.get("children", []), valid_ts_set)
                nd = dict(node); nd["children"] = new_children; result.append(nd)
        return result
    ...
    def _find_folder_node(self, tree_list, name):
        for node in tree_list or []:
            ...
            if "children" in node:
                if found := self._find_folder_node(node["children"], name): return found
        return None
    ...
    def _dedupe_notes_and_fix_tree(self, data) -> bool:
        ...
        seen, remap, changed = set(), {}, False
        unique_notes = []
        for n in notes:
            ts = n.get("timestamp") or ""
            if not ts or ts in seen:
                new_ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                while new_ts in seen: new_ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
        ...
        if changed:
            def fix_tree_timestamps(nodes):
                for node in nodes or []:
                    if node.get("type") == "note":
                        if (ts := node.get("timestamp")) in remap: node["timestamp"] = remap[ts]
                    elif node.get("type") == "folder": fix_tree_timestamps(node.get("children", []))
    ...
    def save_app_data(self, force_container=None):
        ...
        try:
            ...
            for note in ui_notes:
                if note.get("timestamp"): all_notes_dict[note['timestamp']] = note
            ...
            data_to_save = {
                "task_lists": tasks_data, "active_task_list": active_task_list,
                "notes": self.all_notes_cache, "note_tree": self.note_tree_cache,
            }
    ...
    def delete_note_by_timestamp_from_all_data(self, timestamp: str):
        ...
        def find_and_remove_in_tree(nodes):
            ...
            for node in nodes:
                if node.get("type") == "folder": find_and_remove_in_tree(node.get("children", []))
    ...
    def save_zen_note(self, note_timestamp, new_text):
        ...
        # Собираем данные из кеша и сохраняем на диск.
        data_to_save = {
        ...
        except Exception as e: print(f"Ошибка сохранения данных после Zen: {e}")
    ...
    def main_popup_on_data_changed(self):
        ...
        try:
            ...
            if is_dirty: container.on_data_changed()
            else: container.set_status_saved()
        except Exception as e: print(f"main_popup_on_data_changed: {e}")
...
class BackupManagerDialog(QDialog):
    ...
    def populate_backups(self):
        ...
        if not os.path.exists(BACKUP_DIR):
            self.backup_list_widget.addItem(self.loc.get("backup_no_copies")); return
        backups = sorted(glob(os.path.join(BACKUP_DIR, "data_*.bak")), reverse=True)
        if not backups:
            self.backup_list_widget.addItem(self.loc.get("backup_no_copies")); return
        for backup_file in backups:
            item = QListWidgetItem(self.get_date_from_filename(backup_file))
    ...
    def get_date_from_filename(self, filename):
        ...
        except ValueError: return os.path.basename(filename)
    ...
    def delete_selected(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            except OSError as e: QMessageBox.critical(self, "Ошибка", f"Не удалось удалить файл: {e}")
...
if __name__ == "__main__":
    ...
    def update_global_dialog_stylesheet():
        ...
        dialog_stylesheet = f"""
            QDialog, QMessageBox, QInputDialog {{ background-color: {bg}; }}
            QDialog QLabel, QMessageBox QLabel, QInputDialog QLabel {{ color: {text}; background-color: transparent; }}
            QDialog QLineEdit, QMessageBox QLineEdit, QInputDialog QLineEdit {{
                background-color: {comp_bg}; border: 1px solid {border}; border-radius: 4px;
                color: {text}; padding: 5px; min-width: 200px;
            }}
            QDialog QPushButton, QMessageBox QPushButton, QInputDialog QPushButton {{
                background-color: {comp_bg}; color: {text}; border: 1px solid {border};
                padding: 6px 12px; border-radius: 4px; min-width: 80px; min-height: 20px;
            }}
            QDialog QPushButton:hover, QMessageBox QPushButton:hover, QInputDialog QPushButton:hover {{
                background-color: {QColor(comp_bg).lighter(110).name()}; border: 1px solid {accent};
            }}
